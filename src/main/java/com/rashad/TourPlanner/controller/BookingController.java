...
import com.rashad.TourPlanner.service.BookingService;
...
public class BookingController {
    ...
    public ResponseEntity<?> getMyBookings(Principal principal) {
        // 1. Get user email from JWT Principal
        String email = principal.getName();

        return userRepo.findByEmail(email)
                .map(user -> {
                    // 3. Fetch bookings for this user
                    List<Booking> myBookings = bookingRepo.findByCustomer_Id(user.getId());

                    // 4. Wrap in a Map if your frontend expects { "bookings": [...] }
                    // Or return directly if it expects an array [...]
                    return ResponseEntity.ok(Map.of("bookings", myBookings));
                })
