...
import com.rashad.TourPlanner.service.CloudinaryImageService;
import com.rashad.TourPlanner.service.TourService;
...
public class TourController {
    ...
    public TourController(ObjectMapper objectMapper) {
    ...
    @PostMapping
    @PreAuthorize("hasRole('ADMIN')") // Restricting access to users with an ADMIN role
    public ResponseEntity<Tour> addTourWithImages(@RequestParam("tour") String tourJson,
                                                  @RequestParam(value = "image1", required = false) MultipartFile image1,
                                                  @RequestParam(value = "image2", required = false) MultipartFile image2) throws JsonProcessingException {
    ...
    public ResponseEntity<Tour> updateTour(@PathVariable Long id,
        ...
        try {
            Tour updatedTour = objectMapper.readValue(updatedTourJson, Tour.class);
    ...
    public ResponseEntity<Void> deleteTour(@PathVariable Long id) {
        try {
            ...
            Tour tour = (Tour) tourService.getTourById(id).orElseThrow(() -> new RuntimeException("Tour not found"));
